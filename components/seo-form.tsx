...
export function SeoForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const heroBgInput = document.getElementById('hero_bg_image') as HTMLInputElement;
    if (heroBgInput?.files?.[0]) {
      submitFormData.append('hero_bg_image', heroBgInput.files[0]);
    }
    ...
    <Card>
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="hero_bg_image">Hero Background Image (Optional)</Label>
            <Input
              id="hero_bg_image"
              type="file"
              accept="image/*"
              disabled={loadingData}
            />
            {seoData?.hero_bg_image && (
              <p className="text-sm text-muted-foreground">
                Current image: {seoData.hero_bg_image.split('/').pop()}
              </p>
            )}
            <p className="text-sm text-muted-foreground">
              Recommended: High resolution image for hero section background
            </p>
          </div>
